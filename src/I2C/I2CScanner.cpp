...
I2C_Scanner::I2C_Scanner(std::string& i2c_master_path, unsigned int timeout) :
		m_running(true),
		m_timeout(timeout),
...
//		m_scanner_thread->yield();	
		...
		if (m_Condition.wait_for(lock,
			...
			if (i2cbus->i2cOpen(m_I2C_master_path)) {//open or used already open i2c bus
				...
				if (((p_livelist = i2cbus->ScanBus())) != NULL) {//get livelist
					...
					for (slaveID = 0; slaveID < MAX_I2C_SLAVE_ADR; slaveID++) {//check livelist
						...
						curSlave = boost::serialization::singleton<SlaveList>::get_mutable_instance().getSlave(
										slaveID);
							...
							i2c_INFO_WRITE("Slave[0x%x]: found", slaveID);
							{
								...
								if (curSlave == NULL) {//only add not already listed slaves
									boost::serialization::singleton<SlaveList>::get_mutable_instance().addSlave(
											createSlavebyType(slaveID, m_sp_i2cbus_comm));
								}
							}
...
shared_ptr<I2C_Slave> I2C_Scanner::createSlavebyType (uint8_t slaveaddress, I2CSCANNER_H_NS::shared_ptr<I2C::I2C_Comm> i2c_comm)
{
	...
	if(curSlave.get() == NULL)curSlave = shared_ptr<I2C_Slave>(new I2C_Slave(slaveaddress, m_sp_i2cbus_comm));	//default handling
	curSlave-> init();
	return curSlave;
}
